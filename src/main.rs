...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !std::env::args().any(|arg| arg.contains("receive")) {
        ...
        unsafe{libc::ioctl(fd.as_fd().as_raw_fd(), linux::ioctl::VIDIOC_REQBUFS as _, &mut v4l2_requestbuffers{type_: Type::VideoCapture as u32, memory: Memory::Mmap as u32, count: count as u32, ..std::mem::zeroed()} as *mut _ as *mut std::os::raw::c_void)};
        let mut buffer : [_; count] = std::array::from_fn(|index| v4l2_buffer{type_: Type::VideoCapture as u32, memory: Memory::Mmap as u32, index: index as u32, ..unsafe { std::mem::zeroed() }});
        buffer.each_mut().map(|buffer| unsafe{libc::ioctl(fd.as_fd().as_raw_fd(), linux::ioctl::VIDIOC_QUERYBUF as _, buffer as *mut _ as *mut std::os::raw::c_void)});
        ...
        buffer.each_mut().map(|buffer| unsafe{libc::ioctl(fd.as_fd().as_raw_fd(), linux::ioctl::VIDIOC_QBUF as _, buffer as *mut _ as *mut std::os::raw::c_void)});
        ...
        loop {
            ...
            let mut buffer = v4l::v4l2_buffer{type_: Type::VideoCapture as u32, memory: Memory::Mmap as u32, index: index as u32, ..unsafe { std::mem::zeroed() }};
